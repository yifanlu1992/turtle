...
def closest_time(time, timelist, i=0):
    ...
    index = len(timelist)
    indx = int(index/2)
    if not timelist[0] < time < timelist[-1]:
...
for i in indx:
    ...
    Index.sort()
    print '64'
    bottom_index=[]  # all upcast index for each profile
    top_time=[]
    for k in range(len(Index)):
        ...
        if bottom==[]:
            ...
            bottom_index.append(bottom[-1])
            top_time.append(newtime(Index[k][1]))
    #INdex=np.array(INdex).flatten()
    for j in range(len(bottom_index)):
        N=closest_time(obsTime[i],top_time)  #find the nearst index of upcast with profile.
        for k in range(len(dep)):
            if bottom_index[N]==k:
                if 
                INDX=[Index_all[],Index_all[k]]

    plt.figure()     
    plt.plot(temp[INDX[0]:INDX[1]],depth[INDX[0]:INDX[1]],'ro-',linewidth=1)

...
time=pd.Series(secondData['time'][tf_index1],index=tf_index2)
depth=pd.Series(secondData['depth'][tf_index1],index=tf_index2)
temp=pd.Series(secondData['temp'][tf_index1],index=tf_index2)
inde=pd.Series(secondData['index'][tf_index1],index=tf_index2)
indx=[]
for i in tf_index:
...
for i in indx:
    ...
    for j in tf_index2:
        if i==inde[j]:
           Index_all.append(j)
    newdepth=pd.Series(depth[Index_all],index=range(len(Index_all)))
    newtime=pd.Series(time[Index_all],index=range(len(Index_all)))
